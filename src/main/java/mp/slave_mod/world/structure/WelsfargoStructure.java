...
public class WelsfargoStructure extends SlaveModModElements.ModElement {
			...
			if ((random.nextInt(1000000) + 1) <= 100000) {
				...
				for (int a = 0; a < count; a++) {
					...
					Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
							.getTemplateDefaulted(new ResourceLocation("slave_mod", "otrokar"));
					if (template == null)
						return false;
	...
	public WelsfargoStructure(SlaveModModElements instance) {
		super(instance, 149);
	...
	public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
		event.getRegistry().register(feature.setRegistryName("welsfargo"));
	}
